...
// (or admin). Sub-boards inherit their parent board's visibility, so callers
// join on board_id and reuse this set.
export async function getVisibleBoardIds(hasSession: boolean, access: BoardsAccess | null): Promise<string[]> {
  ...
  return rows
    .filter((r) => {
      ...
      if (r.visibility === 'PRIVATE') return !!access?.canModerate
      return false
    })
    .map((r) => r.id)
}
...
export async function isBoardVisible(boardId: string, hasSession: boolean, access: BoardsAccess | null): Promise<boolean> {
  ...
  if (board.visibility === 'PRIVATE') return !!access?.canModerate
  return false
}
