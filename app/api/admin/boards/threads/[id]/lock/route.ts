...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  const nextValue = typeof body.value === 'boolean' ? body.value : !(thread.is_locked as boolean)

  const [updated] = await prisma.$queryRaw<Record<string, unknown>[]>`
    UPDATE "brd_threads" SET "is_locked" = ${nextValue}, "updated_at" = CURRENT_TIMESTAMP WHERE "id" = ${id} RETURNING *
  `
  await logModerationAction({
    actorId: user.id, actorName: user.displayName ?? user.username, action: nextValue ? 'lock' : 'unlock', itemType: 'THREAD', itemId: id,
  })
